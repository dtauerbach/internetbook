...
Our society sees the world through the lens of information, and this fundamental shift in our collective thinking has permeated the standard lexicon. If we are stretched thin at work, we say that we do not have enough \emph{bandwidth} to complete an extra task. If an activity will take too much time, we say that we do not have enough \emph{cycles}. We know roughly how many songs can be stored on a gigabyte thumb drive, or how many megapixels our phone's camera needs to make a good quality photo.

And while computer usage has taught a basic literacy to people all over the world about how information is stored, transferred, and processed, \emph{information theory} as a field of study has infiltrated an incredible number of other academic fields of study. The elegant formulation of information theory, first described by the researcher Claude Shannon in 1948, turns out to be applicable not just to understanding information in computer networks, but also to genetics, economics, molecular biology, electrical engineering, applied physics, and even incredibly abstract branches of theoretical physics and mathematics. It has become a mathematical centerpiece spanning a staggering number of disciplines. Even more amazingly, unlike previous transformative works like Isaac Newton's Philosophiæ Naturalis Principia Mathematica, Claude Shannon's core ideas are not difficult to understand. One of the major goals of this text is to become familiar with the basics of information theory.

Beyond the abstract theoretical underpinnings, there are a host of much more practical questions that one can ask about modern information technology. How do computers work? How does the Internet work? How does my phone work? How does wireless work? Developing a complete understanding of these questions may feel hopelessly out of reach to most readers, but this pessimism is not warranted.

As a rough first cut, let's conceptualize this array of questions about information technology against two broad and interrelated topics: computing and networking.

[GRAPHIC]

If you learn about computing, you will learn about how your laptop works, and how your phone works (since your phone is really just a computer). You will learn about how computers can accomplish useful things like displaying a cute video of a cat on your screen. You will learn about microprocessors, random access memory, hard drives, the components that allow these systems to talk to one another. You will learn about software, programming languages, how code is written, algorithms, computer graphics, Moore's law, and the limits of computing. You will learn about copyright, the battles between free and proprietary software, the role the government played in developing early computers, and the ecosystem of private companies that allow you to go to the store and buy a phone with pre-installed software.
...
In practice these topics are often intertwined in thorny ways and so an individual who is interested in information will invariably learn bits and pieces of both. This book focuses primarily on the networking side of the fence: our goal will NOT be to understand how computing works. For readers without a background in computing who want an excellent practical introduction to the fundamentals of how a computer works, I strongly recommend the book \emph{Code} by Charles Petzold. For those more inclined towards the theoretical puzzles of computing, abstracted away from the hardware, a good introductory text is \emph{The Pattern on the Stone} by W. Daniel Hillis.

While this book is largely focused on networking, it is framed more broadly as a study into the role of technology in human communication throughout history. This framing will allow us to cover [MORE HERE]

Understanding both of these topics together ought to give you a good picture of modern networked computing technology. The goal of this text is to focus on networking, but to frame it a bit more broadly around how humans use technology to communicate. This will allow us to cover a breadth of topics focused around human communication, including the basics of electrical systems, networking, understanding analog and digital systems, networking theory,
...
Chappe's large mechanical arm positions offer one way to communicate at a distance, mimicking the game of catch we played earlier with Mary. What other methods might we use?

In this section, let's briefly consider another method of communication using light. Suppose that your football-playing friend Mary lives several miles away on a hill which is clearly visible from your house. Just after your game of ``catch,'' as you are about to part ways, Mary extends you an open invitation to dinner for being such an excellent throwing partner. She tells you to come any night that you would like -- how nice of her!

Unfortunately, there is a catch. Mary prefers a quiet life and so has no phone or Internet connection. She assures you that she will be reliably cooking every night, and that if you show up around 8 o'clock in the evening, you will be able to enjoy whatever she has prepared; there is no need to let her know in advance that you'll be joining.

But alas, you are a vegetarian, and Mary sometimes likes to cook meat. While you enjoy Mary's company, it's definitely not worth a forty five minute trek to her house unless you know Mary has cooked a meal you can enjoy.

How can Mary communicate what type of meal she has cooked to you so that you know whether or not to come? Building a tower is too much work, and you couldn't see it in the dark anyway, a disadvantage of Chappe's telegraph network to be sure.

But just as you are about to give up, Mary remembers that she has a huge bright light on her roof that when turned on is visible to you from your house. (Light bulbs were not invented until the late 1800s, so this method was not available to Chappe.) If she is cooking a meal with meat, she will shine the light and leave it on all evening. That way you will know that if you see the light on, you should avoid coming over.
...
2 \text{ lights} &= 2*2 = 2^{2} = 4 \text{ configurations} \\
3 \text{ lights} &= 2*2*2 = 2^{3} = 8 \text{ configurations} \\
4 \text{ lights} &= 2*2*2*2 = 2^{4} = 16 \text{ configurations} \\
5 \text{ lights} &= 2*2*2*2*2 = 2^{5} = 32 \text{ configurations} \\
6 \text{ lights} &= 2*2*2*2*2*2 = 2^{6} = 64 \text{ configurations} \\
7 \text{ lights} &= 2*2*2*2*2*2*2 = 2^{7} = 128 \text{ configurations} \\
8 \text{ lights} &= 2*2*2*2*2*2*2*2 = 2^{8} = 256 \text{ configurations} \\
9 \text{ lights} &= 2*2*2*2*2*2*2*2*2 = 2^{9} = 512 \text{ configurations} \\
&\text{etc}
\end{align*}
...
Consider the letter \emph{`a'}, the first letter of the Latin alphabet. What makes this mark meaningful? If we did not know English or any other languages based on this alphabet, then \emph{a} would just appear to be a mysterious mark with no meaning, such as $\includegraphics{randomsymbol}$.
