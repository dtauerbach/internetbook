...
\section{Arm Positions and Information}

We've seen how a semaphore system can be used to create a one-to-one correspondence between arm positions and letters of the alphabet. The actual system used by Chappe's towers was very similar:
...
It is easy to imagine how this could be used to convey entire sentences. However, while the system was general enough to allow for entire sentences, it was often not practical to send full sentences when less information was needed.

To understand why, consider that the time it takes to transmit a message grows with the number of letters, since an operator sitting in a tower has to shift the mechanical arms of the tower for each letter. Let's estimate that it takes 30 seconds to change the position of the large mechanical arms to form a letter, and another 30 seconds of waiting to ensure the next relay station down the line got a good view of the letter. That means it takes 1 minute per letter, and so a sentence made up of 14 letters might take 14 minutes to go from one tower to the next. If there are 30 such towers each 10 miles apart, transmitting your 14-letter message across a distance of 300 miles will take 14 minutes $*$ 30 towers, or 7 hours. Still faster than horseback, but only slightly.

We can greatly improve the efficiency of the system if the operators sitting in the towers of optical telegraphs simultaneously transmitted and received, meaning that they saw a letter, and immediately imitated it for the next tower. This helps, but also makes the system more error-prone, since it requires operators to synchronize their speed, else letters could get missed. Without getting too deep into the tradeoffs between speed and effectiveness in transmission, sending longer messages clearly was challenging.
...
What if the French decided that 36 phrases weren't enough to capture all of the messages that they wanted to send? Of course they could always fall back on using the optical telegraph network to convey full sentences, but before doing that, it makes sense to create codes of 2-characters
